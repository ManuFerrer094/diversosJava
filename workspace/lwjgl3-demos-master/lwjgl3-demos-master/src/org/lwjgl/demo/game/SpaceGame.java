...
package org.lwjgl.demo.game;
                            // Importamos las librerías que vamos a utilizar a continuación en el proyecto
import org.lwjgl.BufferUtils;
import org.lwjgl.PointerBuffer;
import org.lwjgl.demo.opengl.util.WavefrontMeshLoader;                          // WaveFront = obj
import org.lwjgl.demo.opengl.util.WavefrontMeshLoader.Mesh;                     // De esta libreria añade la capacidad de cargar las mallas
import org.lwjgl.glfw.*;
import org.lwjgl.opengl.GL;                                                     // Las librerias que cargan la parte 3D del engine
import org.lwjgl.opengl.GLCapabilities;
import org.lwjgl.opengl.GLUtil;
import org.lwjgl.system.Callback;
import org.joml.FrustumIntersection;                                            // utilidades genericas
import org.joml.GeometryUtils;                                                  // Utilidades de geometria
import org.joml.Intersectiond;
import org.joml.Intersectionf;
import org.joml.Matrix4f;                                                       // operaciones de matrices en cuatro dimensiones
import org.joml.Quaternionf;                                                    // Esto es el infierno de las rotaciones
import org.joml.Vector3d;                                                       // Tratar vectores 
...
import static org.lwjgl.opengl.GL20.*;                                          // Carga de versiones de openGL
 ...
 * @author Kai Burjack
 */
// Una clase dios para hacer un juego de naves espaciales
public class SpaceGame {
    // Space camera que rige la posicion y la transformacion en definitiva de la camara
    private static class SpaceCamera {
        public Vector3f linearAcc = new Vector3f();
        public Vector3f linearVel = new Vector3f();
        public float linearDamping = 0.08f;                                     // Frenada lineal
        ...
        public float angularDamping = 0.5f;                                     // Frenada angular

        public Vector3d position = new Vector3d(0, 0, 10);                      // Posicion inicial de la camara
        public Quaternionf rotation = new Quaternionf();                        // Nuevo quaternion para el giro

        public SpaceCamera update(float dt) {                                   // Actualiza la transformacion de la camara
        ...
        public Vector3f right(Vector3f dest) {                                  // Gestiona el vector derecho
            return rotation.positiveX(dest);
        }
        public Vector3f up(Vector3f dest) {                                     // Gestiona el vector superior
            return rotation.positiveY(dest);
        }
        public Vector3f forward(Vector3f dest) {                                // Gestiona el vector frontal
            return rotation.positiveZ(dest).negate();
        }
    }

    private static class Ship {                                                 // Una pequeña subclase para las naves enemigas
        public double x, y, z;                                                  // Propiedades x y z
        public long lastShotTime;                                               // Propiedad ultima vez que disparó
    }

    private static class Asteroid {                                             // Pequeña subclase asteroide
        public double x, y, z;                                                  // Propiedades de posicion
        public float scale;                                                     // La escala del asteroide
    }

    // Esto de aqui son las condiciones iniciales que hacemos siempre en nuestros juego
    ...
    private void createAsteroid() throws IOException {
        WavefrontMeshLoader loader = new WavefrontMeshLoader();
        asteroid = loader.loadMesh("org/lwjgl/demo/game/miroca.zip");
    ...
    private void drawVelocityCompass() {
        ...
        glEnable(GL_DEPTH_TEST);
        glBegin(GL_LINES);              // Vamos al cachondeo del OpenGL
        glColor4f(1, 0, 0, 1);          // Color rojo opaco        
        glVertex3f(0, 0, 0);            // Empieza en 0
        glVertex3f(1, 0, 0);            // Llega hasta 1,0,0
        glColor4f(1, 0, 0, 1);          // Color rojo opaco 
        glVertex3f(1, 0, 0);
