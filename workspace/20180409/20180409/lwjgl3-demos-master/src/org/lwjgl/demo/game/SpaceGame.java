...
public class SpaceGame {
        public float angulocamara = 0.0f;
        public float distanciacamara = 10.0f;
    ...
    private static class Nervio1 {
        public double x, y, z;
        public float scale;
    }
    
    private static class Nervio2 {
        public double x, y, z;
        public float scale;
    }
    ...
    private int edificioProgram;
    private int edificio_viewUniform;
    private int edificio_projUniform;
    private int edificio_modelUniform;
    private int asteroidProgram;
    private int ship_viewUniform;
    private int ship_projUniform;
    private int ship_modelUniform;
    private int asteroid_viewUniform;
    private int asteroid_projUniform;
    private int asteroid_modelUniform;
    ...
    private Mesh nervio1;
    private int nervio1PositionVbo;
    private int nervio1NormalVbo;
    private Mesh nervio2;
    private int nervio2PositionVbo;
    private int nervio2NormalVbo;
    private int shipCount = 100;
    private int asteroidCount = 1;
    private int edificioCount = 100;
    private int nervio1Count = 1;
    private int nervio2Count = 1;
    private Mesh  edificio;
    private int edificioPositionVbo;
    private int edificioNormalVbo;
    private static float edificioSpread = 1000.0f;
    private static float edificioRadius =0.5f;
    private float maxAsteroidRadius = 20.0f;
    private static float shipSpread = 1000.0f;
    private static float shipRadius = 0.5f;
   ...
   /* private Ship[] ships = new Ship[shipCount];
   
    {
        ...
        for (int i = 0; i < ships.length; i++) {
            ...
            ship.y = (Math.random() * Math.PI*2); // Esta Y se usará como la rotación vertical, no como la posicion
         ...
         int contador = 0;
            for (int x = 0; x < Math.sqrt(edificios.length); x++) {
                for (int z = 0; z < Math.sqrt(edificios.length); z++) {
                Edificio ship = new Edificio();
                   createShip();
                 edificios[contador] = ship;
                 edificios[contador].x = 1+x;
                 edificios[contador].y = 1;
                 edificios[contador].z = 1+z;
                 edificios[contador].scale = 1f;
    ...
    //Aquí está la matriz global publica 
    public   Edificio[] edificios = new Edificio[edificioCount];
     {
            ...
            for (int x = 0; x < Math.sqrt(edificios.length); x++) {
                ...
                Edificio edificio = new Edificio();
                   //createEdificio();
                edificio.x = 0+x * edificioSpread;
                edificio.y = (Math.random() - 0.5) * edificioSpread;

                edificio.y = (Math.random() * Math.PI*2); // Esta Y se usará como la rotación vertical, no como la posicion
                edificio.z = 0+z * edificioSpread;


                edificios[x] = edificio;
            }
        }
    ...
    {
        int contador = 0;
        for (int x = 0; x < Math.sqrt(asteroids.length); x++) {
            for (int z = 0; z < Math.sqrt(asteroids.length); z++) {
            ...
            asteroid.z = 0;
            asteroid.scale = 1;
            asteroids[contador] = asteroid;
            contador++;
            }
        }
    }
    private Nervio1[] nervios1 = new Nervio1[nervio1Count];
    {
        for (int i = 0; i < nervios1.length; i++) {
            Nervio1 nervio1 = new Nervio1();
            /*
    ...
    private Nervio2[] nervios2 = new Nervio2[nervio2Count];
    {
        for (int i = 0; i < nervios2.length; i++) {
            Nervio2 nervio2 = new Nervio2();
            /*
            float scale = (float) ((Math.random() * 0.5 + 0.5) * maxAsteroidRadius);
            asteroid.x = (Math.random() - 0.5) * shipSpread;
            asteroid.y = (Math.random() - 0.5) * shipSpread;
    ...
    private void init() throws IOException {
        ...
        window = glfwCreateWindow(width, height, "GTA Ibiza", !windowed ? monitor : 0L, NULL);
        if (window == NULL) {
   ...
   private void createShip() throws IOException {
      ...
      private void createEdificio() throws IOException {
        WavefrontMeshLoader loader = new WavefrontMeshLoader();
        ship = loader.loadMesh("org/lwjgl/demo/game/edificio001.zip");
        shipPositionVbo = glGenBuffers();
        glBindBuffer(GL_ARRAY_BUFFER, shipPositionVbo);
        glBufferData(GL_ARRAY_BUFFER, ship.positions, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    ...
    private void createAsteroid() throws IOException {
        WavefrontMeshLoader loader = new WavefrontMeshLoader();
        asteroid = loader.loadMesh("org/lwjgl/demo/game/ibiza5.zip");
        // Listado de vertices de todos los objetos:
        System.out.println("La lista de posiciones de los vertices es: "+asteroid.positions);
       
            {
           
        
    }
           //Aqui se creara la ciudad
            for (int i = 0; i < asteroid.objects.size(); i++) 
            {
            // Listado de objetos:
           // Primero los metro en una matriz de objetos
            Object[] listaObjetos = asteroid.objects.toArray();
            // Y luego saco el primero de los objetos por pantalla
            System.out.println("el objeto numero "+i+"es "+listaObjetos[i].toString());
            // Ahora contamos el numero de objetos en el archivo
           // System.out.println("El Objeto  es: "+toArray[i]); 
    ...
    private void createNervio1() throws IOException {
        WavefrontMeshLoader loader = new WavefrontMeshLoader();
        asteroid = loader.loadMesh("org/lwjgl/demo/game/nervio1.zip");
        asteroidPositionVbo = glGenBuffers();
        glBindBuffer(GL_ARRAY_BUFFER, nervio1PositionVbo);
        glBufferData(GL_ARRAY_BUFFER, nervio1.positions, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        asteroidNormalVbo = glGenBuffers();
        glBindBuffer(GL_ARRAY_BUFFER, nervio1NormalVbo);
        glBufferData(GL_ARRAY_BUFFER, nervio1.normals, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
    private void createNervio2() throws IOException {
        WavefrontMeshLoader loader = new WavefrontMeshLoader();
        asteroid = loader.loadMesh("org/lwjgl/demo/game/nervio2.zip");
        asteroidPositionVbo = glGenBuffers();
        glBindBuffer(GL_ARRAY_BUFFER, nervio2PositionVbo);
        glBufferData(GL_ARRAY_BUFFER, nervio2.positions, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        asteroidNormalVbo = glGenBuffers();
        glBindBuffer(GL_ARRAY_BUFFER, nervio2NormalVbo);
        glBufferData(GL_ARRAY_BUFFER, nervio2.normals, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
    ...
    private void createShipProgram() throws IOException {
     ...
     private void createEdificioProgram() throws IOException {
        int vshader = createShader("org/lwjgl/demo/game/ship.vs", GL_VERTEX_SHADER);
        int fshader = createShader("org/lwjgl/demo/game/ship.fs", GL_FRAGMENT_SHADER);
        int program = createProgram(vshader, fshader);
        glUseProgram(program);
        edificio_viewUniform = glGetUniformLocation(program, "view");
        edificio_projUniform = glGetUniformLocation(program, "proj");
        edificio_modelUniform = glGetUniformLocation(program, "model");
        glUseProgram(0);
        edificioProgram = program;
    }
    private void createAsteroidProgram() throws IOException {
        int vshader = createShader("org/lwjgl/demo/game/asteroid.vs", GL_VERTEX_SHADER);
        int fshader = createShader("org/lwjgl/demo/game/asteroid.fs", GL_FRAGMENT_SHADER);
        int program = createProgram(vshader, fshader);
        glUseProgram(program);
        asteroid_viewUniform = glGetUniformLocation(program, "view");
        asteroid_projUniform = glGetUniformLocation(program, "proj");
        asteroid_modelUniform = glGetUniformLocation(program, "model");
        glUseProgram(0);
        asteroidProgram = program;
    }
    ...
    private void update() {
        ...
        glUniformMatrix4fv(ship_projUniform, false, projMatrix.get(matrixBuffer));
         /* Update the edificio shader */
        glUseProgram(edificioProgram);
        glUniformMatrix4fv(edificio_viewUniform, false, viewMatrix.get(matrixBuffer));
        glUniformMatrix4fv(edificio_projUniform, false, projMatrix.get(matrixBuffer));
        
        /* Update the ship shader */
        glUseProgram(asteroidProgram);
        glUniformMatrix4fv(asteroid_viewUniform, false, viewMatrix.get(matrixBuffer));
        glUniformMatrix4fv(asteroid_projUniform, false, projMatrix.get(matrixBuffer));
    ...
    private void updateControls() {
        ...
        if (rightMouseDown)
            ...
            cam.angularAcc.set(0, 2.0f*mouseX*mouseX*mouseX, 0); // Este es control de giro de camara que es que el que hace que la Y este vertical
            //cam.position.set(0.0f,0.0f,3.0f);
...
*/
    ...
    private void drawEdificio()
    {
        glUseProgram(edificioProgram);
        glBindBuffer(GL_ARRAY_BUFFER, edificioPositionVbo);
        glVertexPointer(3, GL_FLOAT, 0, 0);
        glEnableClientState(GL_NORMAL_ARRAY);
        glBindBuffer(GL_ARRAY_BUFFER, edificioNormalVbo);
   ...
   /* private void drawShips() {
        ...
        for (int i = 0; i < ships.length; i++) {
            ...
            if (frustumIntersection.testSphere(x, y, z, shipRadius)) {
                ...
                modelMatrix.rotate((float)ship.y,0,1,0); // Rotación para los coches basada en su posicion Y
    ...
    private void drawAsteroids() { //Aqui se dibuja la base
        glUseProgram(asteroidProgram);
        glBindBuffer(GL_ARRAY_BUFFER, asteroidPositionVbo);
        glVertexPointer(3, GL_FLOAT, 0, 0);
        glEnableClientState(GL_NORMAL_ARRAY);
...
*/
    ...
    private void render() {     // Yo se que el metodo render es el ultimo que se ejecuta
        ...
        drawVelocityCompass();
        drawEdificio();
        //angulocamara += 0.001f;         // Estoy aumentando automaticamente en cada uno de los bucles el angulo de la camara
        //angulocamara += mouseX/100;
        //double coseno = Math.cos((double)angulocamara);
        //double seno = Math.sin((double)angulocamara);
        //cam.position.set(coseno*distanciacamara,0.0f,seno*distanciacamara);     // Esto es lo ultimo de lo ultimo que se ejecuta
        //cam.rotation.set(mouseX, mouseY, mouseX, mouseX);
        //cam.rotation.set(0,mouseX,0,0);
