...
public class SpaceGame {
    ...
    private static class Tierra {
        public double x, y, z;   // Posicion de la tierra
        public float scale;        // Escala para la escala del modelo
    }
    private static class BasePerimetral{
        public double x, y, z;
        public float scale;
    }
    ...
    private static final int numerotierras = 2;
    private static final int numerobases = 1;
    private long window;
    private int width = 1920;
    private int height = 1080;
    private int fbWidth = 1920;
    private int fbHeight = 1080;
    ...
    private Mesh tierra;
    private Mesh ibizaPlano;
    private int asteroidPositionVbo;
    private int asteroidNormalVbo;
    private int tierraPositionVbo; //Preguntar que es VBO
    private int tierraNormalVbo;
    private int basePositionVbo; //Preguntar que es VBO
    private int baseNormalVbo;
    ...
    private Tierra[] tierras = new Tierra[numerotierras];
    {
        for (int i = 0; i < tierras.length; i++) {
            Tierra tierra = new Tierra();
            float scale = (float) ((Math.random() * 0.5 + 0.5) * maxAsteroidRadius)*10;
            tierra.x = (Math.random() - 0.5) * shipSpread;
            tierra.y = (Math.random() - 0.5) * shipSpread;
            tierra.z = (Math.random() - 0.5) * shipSpread;
            tierra.scale = scale;
            tierras[i] = tierra;
        }
    }
      
    private BasePerimetral[] listadebases = new BasePerimetral[numerobases];
    {
        for (int i = 0; i < listadebases.length; i++) {
            BasePerimetral base = new BasePerimetral();
    ...
    private void createAsteroid() throws IOException {
     ...
     private void createTierra() throws IOException {
        WavefrontMeshLoader loader = new WavefrontMeshLoader();
        tierra = loader.loadMesh("org/lwjgl/demo/game/traca.obj.zip");         // Cargame el obj
        tierraPositionVbo = glGenBuffers();                                     // Lee sus caras y sus vectores normales
        glBindBuffer(GL_ARRAY_BUFFER, tierraPositionVbo);                       // Escribemelo en OpenGL
        glBufferData(GL_ARRAY_BUFFER, tierra.positions, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        tierraNormalVbo = glGenBuffers();
        glBindBuffer(GL_ARRAY_BUFFER, tierraNormalVbo);
        glBufferData(GL_ARRAY_BUFFER, tierra.normals, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
        private void createBase() throws IOException {
        WavefrontMeshLoader loader = new WavefrontMeshLoader();
        ibizaPlano = loader.loadMesh("org/lwjgl/demo/game/tierra.obj.zip");     // Cargame el obj
        basePositionVbo = glGenBuffers();                                     // Lee sus caras y sus vectores normales
        glBindBuffer(GL_ARRAY_BUFFER, basePositionVbo);                       // Escribemelo en OpenGL
        glBufferData(GL_ARRAY_BUFFER, ibizaPlano.positions, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        baseNormalVbo = glGenBuffers();
        glBindBuffer(GL_ARRAY_BUFFER, baseNormalVbo);
        glBufferData(GL_ARRAY_BUFFER, ibizaPlano.normals, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
    ...
    private void drawTierra() {
        glUseProgram(shipProgram);
        glBindBuffer(GL_ARRAY_BUFFER, tierraPositionVbo);
        glVertexPointer(3, GL_FLOAT, 0, 0);
        glEnableClientState(GL_NORMAL_ARRAY);
        glBindBuffer(GL_ARRAY_BUFFER, tierraNormalVbo);
        glNormalPointer(GL_FLOAT, 0, 0);
        ...
        for (int i = 0; i < tierras.length; i++) {
            ...
            if (tierra == null)
                continue;
            float x = (float)(tierra.x - cam.position.x);
            float y = (float)(tierra.y - cam.position.y);
            float z = (float)(tierra.z - cam.position.z);
            if (frustumIntersection.testSphere(x, y, z, tierra.scale)) {
                modelMatrix.translation(x, y, z);
                modelMatrix.scale(tierra.scale);
                glUniformMatrix4fv(ship_modelUniform, false, modelMatrix.get(matrixBuffer));
                glDrawArrays(GL_TRIANGLES, 0, this.tierra.numVertices);
            }
        }
        glDisableClientState(GL_NORMAL_ARRAY);
    }
     private void drawBase() {
        glUseProgram(shipProgram);
        glBindBuffer(GL_ARRAY_BUFFER, basePositionVbo);
        glVertexPointer(3, GL_FLOAT, 0, 0);
        glEnableClientState(GL_NORMAL_ARRAY);
        glBindBuffer(GL_ARRAY_BUFFER, baseNormalVbo);
        glNormalPointer(GL_FLOAT, 0, 0);
        ...
        for (int i = 0; i < listadebases.length; i++) {
            BasePerimetral bp = listadebases[i];
                bp.scale = 100;
            float x = (float)(150- cam.position.x);
            float y = (float)(150 - cam.position.y);
            float z = (float)(150- cam.position.z);
            if (frustumIntersection.testSphere(x, y, z, bp.scale)) {
                modelMatrix.translation(x, y, z);
                modelMatrix.scale(bp.scale);
                glUniformMatrix4fv(ship_modelUniform, false, modelMatrix.get(matrixBuffer));
                glDrawArrays(GL_TRIANGLES, 0, this.tierra.numVertices);
            }
        }
